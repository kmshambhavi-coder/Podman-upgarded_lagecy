...
|Script Timeout (Seconds)|Timeout limit for the python process running the current script.|True|480|
...
|Verify SSL|Verify SSL certificates for HTTPS requests to Microsoft Azure.||true|
...
|Total number of Scheduled Alerts Events Limit to Ingest|Specify the total number of events the connector should ingest for a Microsoft Sentinel incident that is based on a Scheduled or NRT alert. Connector ‘counts’ events in all corresponding SecOps alerts created for the Microsoft Sentinel Incident and no new events will be ingested once this limit is reached.||100|
