...
|Insight Name|Specify the new name of the insight.||String||
|Group By Attribute|Specify a new name of the attribute by which findings should be grouped under one insight.||List||
|Filter JSON Object|Specify a new filter for the findings. Filter is represented as a JSON object, where you can specify different attributes and values. Please refer to action documentation for more details.||String||
...
|----|-----------|-----------|----|------------|
|Insight ARN|Specify the ARN of the insight.|True|String||
|Max Results to Return|Specify how many results to return.||String||
...
|Note|Specify new text for the note on the finding. Note: If the “Note” parameter has a value, you should also fill out the “Note Author” parameter. This is an AWS Security Hub limitation.||String||
|Note Author|Specify the author of the note. Note: If the “Note Author” parameter has a value, you should also fill out the “Note” parameter. This is an AWS Security Hub limitation.||String||
|Severity|Specify new severity for the finding.||List||
|Verification State|Specify a new verification state for the finding.||List||
|Confidence|Specify new confidence for the finding.||String||
|Criticality|Specify new criticality for the finding.||String||
|Types|Specify a comma-separated list of types for the finding. Example: type1,type2||String||
|Workflow Status|Specify new workflow status for the finding.||List||
|Custom Fields|Specify custom fields that should be updated in the finding. Format Custom_field_1:value,Custom_field_2:value||String||
...
|Verify SSL|If enabled, verify the SSL certificate for the connection to the AWS Security Hub is valid.||Boolean|false|
